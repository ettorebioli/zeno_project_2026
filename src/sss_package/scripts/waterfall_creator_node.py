...
class WaterfallCreatorNode:
	...
	# definire parametri per altri plots
        ...
        # Cartelle per plot finali salvati allo shutdown del nodo
        default_echo_dir = os.path.join(results_dir, "99_echo_intensity")
        self.echo_plot_folder = rospy.get_param('~echo_plot_folder', default_echo_dir)

        default_altitude_dir = os.path.join(results_dir, "99_altitude_plots")
...
# ________________________________________________________________________________________________________________________________
	...
	# opzionale: salvare intensita prima e dopo applicazione TVG per plot
        ...
        waterfall = self.build_waterfall(window)
...
# ________________________________________________________________________________________________________________________________
    ...
    def publish_realtime_waterfall(self, waterfall):
        ...
        ros_img.header.seq = self.ping_index
        ros_img.header.stamp = rospy.Time.now()
        ros_img.header.frame_id = "sss_waterfall_realtime"
        ...
        metadata_msg.ping_indices = [metadata['ping_index'] for metadata in live_metadata]
        metadata_msg.ping_stamps = [metadata['ping_stamp'] for metadata in live_metadata]
        metadata_msg.nav_statuses = [metadata['nav_status'] for metadata in live_metadata]
        metadata_msg.nav_valid = [metadata['nav_valid'] for metadata in live_metadata]
        metadata_msg.altitudes = [metadata['altitude'] for metadata in live_metadata]
        metadata_msg.altitude_valid = [metadata['altitude_valid'] for metadata in live_metadata]
    ...
    def build_image(self, ping_buffer):
        ...
        if len(ping_buffer) >= self.ping_per_image:
            ...
            if self.last_image_ping_index > 0 and (self.ping_index - self.last_image_ping_index) < image_step:
                return None
    ...
    def publish_image(self, image, image_metadata):
        ...
        if not saved:
            rospy.logerr("waterfall_creator_node: impossibile salvare immagine waterfall {}".format(filename))
        self.image_index += 1
